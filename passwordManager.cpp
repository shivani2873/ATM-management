...
void PasswordManager::loadFromFile(){
    ifstream file(LoginName + ".txt");
    if (file.is_open()){
        getline(file, LoginName);
        getline(file, LoginPass);
        User user;
...
void PasswordManager::addUser(){
    ...
    passCheck(newUser.password);  //imp
...
void PasswordManager::passCheck(const string password){
...
void PasswordManager::registerUser(){
    ...
    if (file.is_open()){
        ...
        LoginName = inputUserName;
        LoginPass = inputPassword;
        saveToFile();
